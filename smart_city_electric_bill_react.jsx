...
function renderGlobalHistory() {
  ...
  transactions.forEach(t=>{
    html+=`<tr class="border-t"><td class="py-2">${new Date(t.date).toLocaleString()}</td><td class="py-2">${t.name} <span class="text-xs text-slate-400">(${t.customerId})</span></td><td class="py-2 text-right">₹${t.amount}</td></tr>`;
  });
  html+='</tbody></table>';
...
function showUserPanel(){
  ...
  panel.innerHTML=`
    <div class="flex justify-between items-start">
      <div>
        <h2 class="text-xl font-medium">Welcome, ${user.name}</h2>
        <p class="text-sm text-slate-600">Customer ID: <span class="font-mono">${user.customerId}</span></p>
        <p class="text-sm text-slate-600">Meter No: <span class="font-mono">${user.meter}</span></p>
      </div>
      <div>
        <button id="logoutBtn" class="px-3 py-1 rounded-lg border">Logout</button>
      </div>
    </div>
    <div class="p-3 border rounded-lg bg-slate-50 mt-3">
      <h3 class="font-semibold">Recharge Meter</h3>
      <form id="rechargeForm" class="mt-2 flex gap-2 items-center">
        <input type="number" min="1" value="100" id="rechargeAmt" class="p-2 rounded-md border w-full">
        <button class="px-4 py-2 rounded-lg bg-emerald-600 text-white">Pay</button>
      </form>
      <p class="text-xs text-slate-500 mt-2">This demo simulates a successful recharge and stores history locally in your browser.</p>
      <div id="rechargeError" class="text-red-600 mt-2"></div>
    </div>
    <div class="p-3 border rounded-lg mt-3">
      <h3 class="font-semibold">Last recharges (most recent)</h3>
      <ul id="userTxList" class="mt-2 space-y-2 text-sm text-slate-700"></ul>
    </div>
  `;
